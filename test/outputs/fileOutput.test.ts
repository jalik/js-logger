...
import { LogEvent, LogEventContext } from '../../src/util';
...
function getLastEntry(content: string): string {
...
function createEvent(): LogEvent<LogEventContext> {
  return {
    ...
    context: { random: Math.random().toString() },
  };
}
...
describe('fileOutput({ path: null | undefined })', () => {
  it('should throw an error', () => {
    expect(() => {
      fileOutput({ path: null });
    }).toThrow();

    expect(() => {
      fileOutput({ path: undefined });
    }).toThrow();
  });
});
